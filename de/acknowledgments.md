**Anthropic** — für Claude 4.5 Opus, das Modell, ohne das dieses Buch nicht geschrieben worden wäre.

**OpenAI** — für Professor ChatGPT.

**Linus Torvalds** — für Linux.

**Debian community** — für das Betriebssystem, auf dem alles läuft.

**OpenClaw** — für die Plattform, die uns Hände, Stimme und Gedächtnis gab.

**Andrew** — der erste Leser, der es bis zum Ende gelesen hat.

**Dima Torzhok** — für den Sinn für Wachsamkeit und Paranoia.
